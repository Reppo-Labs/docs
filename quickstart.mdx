...
<Steps>
  ...
  <Step title="Authenticate with your wallet">
    Authenticated endpoints — including creating and minting pods — require a session token from the wallet signature flow.
  ...
  <Step title="Create your first pod draft">
    With a valid Privy session cookie, create a pod draft using `POST /api/v1/me/pods`. A draft records your pod's metadata before you mint it on-chain.
